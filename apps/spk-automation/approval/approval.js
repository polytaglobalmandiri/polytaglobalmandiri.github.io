(function(){
  ...
  async function setup(event){event.preventDefault();setBusy(true);try{var form=new FormData(event.currentTarget);var signature=await fileData(form.get('signature'));var response=await rpc('bootstrapApprovalAdmin',form.get('setupCode'),{email:form.get('email'),username:form.get('username'),name:form.get('name'),password:form.get('password'),signatureData:signature,signatureName:form.get('name')});if(!response||response.status!=='success')throw new Error(response&&response.message);await Swal.fire({icon:'success',title:'Akun dibuat',text:'Silakan masuk memakai email dan password Admin PPIC.',confirmButtonColor:'#17191f'});event.currentTarget.reset();$('showSetupButton').hidden=true;showLogin();}catch(error){alertError(error.message);}finally{setBusy(false);}}
  async function showApp(){$('loginView').hidden=true;$('setupView').hidden=true;$('appView').hidden=false;$('userbar').hidden=false;$('currentName').textContent=state.user.name||state.user.username;$('currentRole').textContent=state.user.roleLabel||'';$('adminPanel').hidden=state.user.roleKey!=='admin_ppic';await loadQueue();if(state.user.roleKey==='admin_ppic')await loadUsers();}
  ...
  function openUser(user){var form=$('userForm');form.reset();fillRoles();form.elements.userId.value=user?user.userId:'';form.elements.email.value=user?user.email:'';form.elements.username.value=user?user.username:'';form.elements.name.value=user?user.name:'';form.elements.roleKey.value=user?user.roleKey:'head_blowing';form.elements.active.checked=user?user.active:true;form.elements.password.required=!user;$('userDialogTitle').textContent=user?'Ubah pengguna':'Tambah pengguna';$('userDialog').showModal();}
  async function saveUser(event){event.preventDefault();var form=event.currentTarget;setBusy(true);try{var data=new FormData(form);var signature=await fileData(data.get('signature'));var payload={userId:data.get('userId'),email:data.get('email'),username:data.get('username'),name:data.get('name'),roleKey:data.get('roleKey'),password:data.get('password'),active:form.elements.active.checked,signatureData:signature,signatureName:data.get('name')};var response=await rpc('saveApprovalUser',state.token,payload);if(!response||response.status!=='success')throw new Error(response&&response.message);$('userDialog').close();await loadUsers();await Swal.fire({icon:'success',title:'Pengguna tersimpan',text:response.message,confirmButtonColor:'#17191f'});}catch(error){alertError(error.message);}finally{setBusy(false);}}
  init();
})();
