...
(function () {
  ...
  function requestDatabase(forceRefresh) {
    return new Promise(function (resolve, reject) {
      if (!hasRuntime()) {
        reject(new Error('Koneksi Database V2 tidak tersedia.'));
        return;
      }
      stepProgress(10, 'Menghubungi Database V2');
      google.script.run
        .withSuccessHandler(function (data) {
          if (!data || data.error) {
            reject(new Error(data && data.error || 'Format Database V2 tidak dikenali.'));
            return;
          }
  ...
  function requestTrackingData() {
    return new Promise(function (resolve, reject) {
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          if (!data || data.error) {
            reject(new Error(data && data.error || 'Tracking Database V2 tidak dikenali.'));
            return;
          }
          resolve(data);
        })
  ...
  function showError(err) {
    setStatus('<i class="fa-solid fa-triangle-exclamation"></i> Database V2 gagal dimuat', true);
    ...
    return requestDatabase(Boolean(forceRefresh)).then(function (payload) {
      ...
      return new Promise(function (resolve) {
        showAll(withProgress, function () {
          setStatus('<i class="fa-solid fa-circle-check"></i> Database V2 live');
          endProgress();
          resolve();
        });
      });
  ...
  function previewTrackingChange() {
    return requestTrackingData().then(function (data) {
      ...
      // boleh membatalkan sinkronisasi seluruh Database V2.
  ...
  function syncChangedDatabase() {
    ...
    setStatus('<i class="fa-solid fa-arrows-rotate fa-spin"></i> Menyinkronkan perubahan Database V2');
  ...
  function load() {
    ...
    startProgress('Menghubungi Database V2');
